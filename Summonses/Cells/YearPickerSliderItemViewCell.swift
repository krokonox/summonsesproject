...
class YearPickerSliderItemViewCell: UIView {
    ...
    override func setSmoothSelected(_ selected: Bool) {
        ...
        self.color = .white//selected ? #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1) : #colorLiteral(red: 0.8406969905, green: 0.8407167792, blue: 0.8407061696, alpha: 1)
    
        loadFirstTime = true
        if giftValueLabel != nil {
            self.giftValueLabel.textColor = color
            self.giftValueLabel.alpha = selected ? 1.0 : 0.4
            self.giftValueLabel.font = selected ? UIFont.systemFont(ofSize: 14.0, weight: .bold) : UIFont.systemFont(ofSize: 14.0)
        }
    }
}
